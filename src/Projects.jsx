import { motion } from "framer-motion";
const Projects = () => {
  return (
    <div className="bg-black bg-opacity-20 mt-56 rounded-md">
      <h1 className="text-3xl lg:text-6xl  pl-6 py-8 font-serif text-orange-400 underline">
        Projects
      </h1>

      <div className=" grid grid-cols-1 lg:grid-cols-3 gap-y-5 pb-8 px-4">
        {/* project1 */}
        <motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }}>
          <div className="rounded-md lg:w-96 bg-gray-500 bg-opacity-20 shadow-xl ">
            <figure className="h-60">
              <img
                src="https://i.ibb.co/yynW5jK/newspaper.png"
                alt="Shoes"
                className="rounded-md"
              />
            </figure>
            <div className="card-body -mt-14 lg:-mt-5 h-60 lg:h-56 ">
              <h2 className="card-title">News Paper Portal</h2>
              <p className="text-justify">
                This is a newpaper website name global Times. It has many more
                features. Here is some features: Login, registration with google
                login. Subscription system for premium user. Show articles, add
                articles, articles are difference for premium user and other
                user.
              </p>
            </div>
            <div className="card-body flex-grow">
              <a
                href="https://newspaper-final-assignment.netlify.app/"
                className="btn-sm bg-green-400 rounded-full pt-1 mb-1  lg:mb-0 lg:btn lg:bg-green-400 lg:rounded-full justify-start text-black lg:text-black lg:hover:text-white"
                rel="noopener noreferrer"
              >
                Live Link
              </a>
              <a
                href="https://github.com/AlMamunTalukder/newspaper-client"
                className="btn-sm bg-gray-400 rounded-full pt-1 lg:btn lg:bg-gray-400 lg:rounded-full justify-end text-white lg:text-white"
                rel="noopener noreferrer"
              >
                Github Link
              </a>
            </div>
          </div>
        </motion.button>
        {/* project2 */}
        <motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }}>
          <div className="rounded-md lg:w-96 bg-gray-500 bg-opacity-20 shadow-xl ">
            <figure className="h-60">
              <img
                src="https://i.ibb.co/TmzXqkt/service.png"
                alt="Service"
                className="rounded-md"
              />
            </figure>
            <div className="card-body -mt-14 lg:-mt-5 h-56">
              <h2 className="card-title ">Service Provide Website</h2>
              <p className="text-justify">
                This is a service provide website name Repair Service Hub. Some
                Top Features are - Log in, Registration service add, Service
                edit, Service update, Service delete. See my service and also
                see my booking service. See the Details.
              </p>
            </div>
            <div className="card-body flex-grow">
              <a
                href="https://fabulous-squirrel-c63dda.netlify.app/"
                className="btn-sm bg-green-400 rounded-full pt-1 mb-1 lg:mb-0 lg:btn lg:bg-green-400 lg:rounded-full justify-start text-black lg:text-black lg:hover:text-white"
                rel="noopener noreferrer"
              >
                Live Link
              </a>
              <a
                href="https://github.com/AlMamunTalukder/service-sharing-client"
                className="btn-sm bg-gray-400 rounded-full pt-1 lg:btn lg:bg-gray-400 lg:rounded-full justify-end text-white lg:text-white"
                rel="noopener noreferrer"
              >
                Github Link
              </a>
            </div>
          </div>
        </motion.button>
        {/* project3 */}
        <motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }}>
          <div className="rounded-md lg:w-96 bg-gray-500 bg-opacity-20 shadow-xl ">
            <figure className="h-60">
              <img
                src="https://i.ibb.co/SBqLsGC/car-brand-shop.png"
                alt="Shoes"
                className="rounded-md"
              />
            </figure>
            <div className="card-body -mt-14 lg:-mt-5 h-56">
              <h2 className="card-title ">Brand Shop Portal</h2>
              <p className="text-justify">
                This is a Automotive project where try to create a car website
                with various brands. In Below, there is at least 5 points of my
                project - Navbar with Login registration 6 brands of famous car.
                Show specific car when click on brand name.
              </p>
            </div>
            <div className="card-body flex-grow">
              <a
                href="https://car-brand-shop.netlify.app"
                className="btn-sm bg-green-400 rounded-full pt-1 mb-1 lg:mb-0 lg:btn lg:bg-green-400 lg:rounded-full justify-start text-black lg:text-black lg:hover:text-white"
                rel="noopener noreferrer"
              >
                Live Link
              </a>
              <a
                href="https://github.com/AlMamunTalukder/brandshop-client"
                className="btn-sm bg-gray-400 rounded-full pt-1 lg:btn lg:bg-gray-400 lg:rounded-full justify-end text-white lg:text-white"
                rel="noopener noreferrer"
              >
                Github Link
              </a>
            </div>
          </div>
        </motion.button>
        {/* project4 */}
        <motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }}>
          <div className="rounded-md lg:w-96 bg-gray-500 bg-opacity-20 shadow-xl ">
            <figure className="h-60">
              <img
                src="https://i.ibb.co/FKtXL81/edutechpro9.png"
                alt="EduTechPro"
                className="rounded-md"
              />
            </figure>
            <div className="card-body -mt-14 lg:-mt-5 h-56">
              <h2 className="card-title text-left ">
                Educational Events Management System
              </h2>
              <p className="text-justify">
                This website is educational events management system name
                edutechpro. Mainly has Some Features- - Educational events add.
                - show details - add to cart
              </p>
            </div>
            <div className="card-body flex-grow">
              <a
                href="https://edutechpro9.netlify.app/"
                className="btn-sm bg-green-400 rounded-full pt-1 mb-1 lg:mb-0 lg:btn lg:bg-green-400 lg:rounded-full justify-start text-black lg:text-black lg:hover:text-white"
                rel="noopener noreferrer"
              >
                Live Link
              </a>
              <a
                href="https://github.com/AlMamunTalukder/edu_tech_pro"
                className="btn-sm bg-gray-400 rounded-full pt-1 lg:btn lg:bg-gray-400 lg:rounded-full justify-end text-white lg:text-white"
                rel="noopener noreferrer"
              >
                Github Link
              </a>
            </div>
          </div>
        </motion.button>
        {/* project5 */}
        <motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }}>
          <div className="rounded-md lg:w-96 bg-gray-500 bg-opacity-20 shadow-xl ">
            <figure className="h-60">
              <img
                src="https://i.ibb.co/t8k1fsr/donationcapm.png"
                alt="Shoes"
                className="rounded-md"
              />
            </figure>
            <div className="card-body -mt-14 lg:-mt-5 h-56">
              <h2 className="card-title">Donation Site</h2>
              <p className="text-justify">
                This is a donation website name donation campaign. The 3 project
                feature is. Show Data in cards, Details Page, Donates Cards show
                in donation page. Statastics, Search functionality.
              </p>
            </div>
            <div className="card-body flex-grow">
              <a
                href="http://donationcapm-2023.surge.sh/"
                className="btn-sm bg-green-400 rounded-full pt-1 mb-1 lg:mb-0 lg:btn lg:bg-green-400 lg:rounded-full justify-start text-black lg:text-black lg:hover:text-white"
                rel="noopener noreferrer"
              >
                Live Link
              </a>
              <a
                href="https://github.com/AlMamunTalukder/donation-campaign"
                className="btn-sm bg-gray-400 rounded-full pt-1 lg:btn lg:bg-gray-400 lg:rounded-full justify-end text-white lg:text-white"
                rel="noopener noreferrer"
              >
                Github Link
              </a>
            </div>
          </div>
        </motion.button>

        {/* project6 */}
        <motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }}>
          <div className="rounded-md lg:w-96 bg-gray-500 bg-opacity-20 shadow-xl ">
            <figure className="h-60">
              <img
                src="https://i.ibb.co/R6HDW0P/course.png"
                alt="Course"
                className="rounded-md"
              />
            </figure>
            <div className="card-body -mt-14 lg:-mt-5 h-56">
              <h2 className="card-title">Course Registration</h2>
              <p className="text-justify">
                This is a newpaper website name global Times. It has many more
                features. Here is some features: Login, registration with google
              </p>
            </div>
            <div className="card-body flex-grow">
              <a
                href="https://course-r2023.surge.sh/"
                className="btn-sm bg-green-400 rounded-full pt-1 mb-1 lg:mb-0 lg:btn lg:bg-green-400 lg:rounded-full justify-start text-black lg:text-black lg:hover:text-white"
                rel="noopener noreferrer"
              >
                Live Link
              </a>
              <a
                href="https://github.com/AlMamunTalukder/course-registration"
                className="btn-sm bg-gray-400 rounded-full pt-1 lg:btn lg:bg-gray-400 lg:rounded-full justify-end text-white lg:text-white"
                rel="noopener noreferrer"
              >
                Github Link
              </a>
            </div>
          </div>
        </motion.button>
      </div>
    </div>
  );
};

export default Projects;
